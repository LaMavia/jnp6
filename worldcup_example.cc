...
public:
  ...
  void onTurn(std::string const &playerName, std::string const &status,
              std::string const &currentSquareName,
              unsigned int money) override {
    info << playerName << " [" << status << "] [" << money << "] - "
         << currentSquareName << "\n";
  }
  ...
  std::string str() { return info.str(); }
};

int main() {
  std::shared_ptr<Die> die1 = std::make_shared<ExampleDie>();
  std::shared_ptr<Die> die2 = std::make_shared<ExampleDie>();
  std::shared_ptr<TextScoreBoard> scoreboard =
      std::make_shared<TextScoreBoard>();
